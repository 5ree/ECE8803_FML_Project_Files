...
def generate_binary_mask(mask_img):
    THRESHOLD = 5
    mask_img = mask_img.mean(axis=2)
    return (mask_img > THRESHOLD).astype(np.uint8) * 255
...
def inference_image(model, device, img_path, img2_paths, tgt2_paths, out_path, mask_path):
    ...
    bin_mask = generate_binary_mask(mask)
    mask_image = Image.fromarray(bin_mask.astype(np.uint8))
